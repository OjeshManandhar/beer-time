...
function Card({ beer }: Props) {
  return (
    <>
      <li className='flex flex-row h-40 gap-5 p-6 overflow-hidden bg-white rounded cursor-pointer drop-shadow-md hover:bg-alice-blue'>
        <img
          id={`all-beer-img-${beer.id}`}
          src={beer.image_url}
          alt={beer.name}
          className='hidden h-full min-w-[75px] max-w-[75px] object-contain sm:block'
        />
      ...
      <Tooltip
        ...
        delayShow={500}
        place='top'
        className='max-w-[250px] px-2 py-1 bg-raisin-black'
      >
